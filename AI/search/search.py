...
def general_search(problem,fringe,add_to_fringe_fn):
    closed=set()
    ...
    while not fringe.isEmpty():
        ...
        if not node in closed:
            ...
            for sub_node,sub_action,sub_cost in problem.getSuccessors(node):
                new_cost = cost+sub_cost
                new_path = path+[sub_action]
                new_state = (sub_node,new_cost,new_path)
...
def depthFirstSearch(problem):
    ...
    def add_to_fringe_fn(fringe,state,cost):
        fringe.push(state)
    return general_search(problem,fringe,add_to_fringe_fn)
